...
<tag:template id="text" name="text" title="Listagem Ordem de Serviço">
	<jsp:attribute name="divBody">
		<script>
			...
			function geraNF(idOrdem, status){
				if (status != '${statusFinalizado}'){
					$("#idOrdemServicoFinaliza").val(idOrdem);
					$("#gridFinalizaModal").modal();
				} else {
					window.location.href="${pageContext.request.contextPath}/protect/formNotaFiscal?id="+idOrdem;
				}	
			}
		</script>	
		...
		<table class="table table-striped" id="lista">
			...
			<tr>
				<td  width="100px">
					<div>
						<b><fmt:message key="ordemServico.id"/></b>
				...
				<td width="130px">
					<div>
						<b><fmt:message key="ordemServico.data"/></b>
						<input type="text" name="filt2" class="form-control" placeholder="Buscar" onKeyUp="filter(this, 'lista', '4')"/>
					</div>
				</td>
				<td colspan="3"></td>	
			</tr>
			...
			<c:if test="${empty ordemServicos}">
				<tr>
					<td colspan="7"><fmt:message key="ordemServico.nenhum.registro"/></td>
				</tr>
			</c:if>
			...
			<c:forEach items="${ordemServicos}" var="ordemServico">
				<tr>
					...
					<td><a href='javascript:void(0);' onclick="javascript:geraNF(${ordemServico.idOrdemServico}, '${ordemServico.status}');" title="Gerar NF"><span class="glyphicon glyphicon-export"></span></a></td>					
				</tr>
			</c:forEach>
		</table>
		<br><br>
		...
		<!-- Modal finalizar OS -->
		<form:form action="${pageContext.request.contextPath}/protect/formNFRedirect" method="post">
			<div id="gridFinalizaModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="gridModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			      	<input type="hidden" id="idOrdemServicoFinaliza" name="id">
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			        <h4 class="modal-title" id="gridModalLabel"><fmt:message key="ordemServico.title"/></h4>
			      </div>
			      <div class="modal-body">
				      	<h4 class="modal-title" id="gridModalLabel" align="center"><img src="${pageContext.request.contextPath}/bootstrap-dist/img/warning-icon.png" width="50px"><fmt:message key="ordemServico.motal.titulo.finaliza"/></h4>
				      </div>
